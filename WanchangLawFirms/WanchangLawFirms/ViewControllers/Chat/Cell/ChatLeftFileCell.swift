...
class ChatLeftFileCell: ChatLeftBaseCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        _ = fileLogoBtn.sd_layout()?.leftSpaceToView(bubbleImgView, kBubbleSpaceL)?.centerYEqualToView(bubbleImgView)?.widthIs(50)?.heightIs(50)
//        _ = fileNameLab.sd_layout()?.leftSpaceToView(fileLogoBtn, kLeftSpaceSS)?.rightSpaceToView(bubbleImgView, kBubbleSpaceS)?.topEqualToView(fileLogoBtn)?.bottomSpaceToView(bubbleImgView, kBubbleSpaceS)
        _ = fileNameLab.sd_layout()?.leftSpaceToView(fileLogoBtn, kLeftSpaceSS)?.rightSpaceToView(bubbleImgView, kBubbleSpaceS)?.topEqualToView(fileLogoBtn)?.heightIs(38)
