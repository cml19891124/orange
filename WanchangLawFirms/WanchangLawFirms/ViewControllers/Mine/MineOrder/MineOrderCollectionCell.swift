...
class MineOrderCollectionCell: UICollectionViewCell {
    ...
    private lazy var listView: JTabNotiView = {
        ...
        tabView.register(MineOrderCell.self, forCellReuseIdentifier: "MineOrderCell")
...
extension MineOrderCollectionCell: JTabNotiViewDataSource {
...
extension MineOrderCollectionCell: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier:"MineOrderCell", for: indexPath) as! MineOrderCell
        cell.delegate = self
        cell.status = status
        cell.model = model
        return cell
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        let model = dataArr[indexPath.section]
        let tempH = kLeftSpaceS * 2 + 65
        var h = model.desc.height(width: kDeviceWidth - kLeftSpaceS - kAvatarWH - 40, font: kFontMS) + tempH
        if h < tempH + 20 {
            h = tempH + 20
        } else if h > tempH + 60 {
            h = tempH + 60
...
extension MineOrderCollectionCell: MineOrderCellDelegate {
